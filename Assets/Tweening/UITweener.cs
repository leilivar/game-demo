...
public abstract class UITweener : MonoBehaviour
{
	...
	float m_duration = 0f;
	float m_amountPerDelta = 1000f;
	float m_factor = 0f;
    float m_firstDelta = 0f;
	...
	public float amountPerDelta
	{
		get
		{
			...
			{
				m_duration = duration;
				m_amountPerDelta = Mathf.Abs(1f / duration) * Mathf.Sign(m_amountPerDelta);
			}
	...
	void Update ()
	{
		...
		m_factor += (duration == 0f) ? 1f : amountPerDelta * delta;
	...
	static public T Begin<T> (GameObject go, float duration) where T : UITweener
	{
		...
		if (comp == null)
		{
            ...
            Assert.IsNotNull(comp, "Unable to add " + typeof(T) + " to " + go.name);
		}

		comp.m_started = false;
		comp.m_factor = 0f;
		comp.duration = duration;
		comp.m_duration = duration;
		comp.m_amountPerDelta = duration > 0f ? Mathf.Abs(1f / duration) : 1000f;
    ...
    public UITweener Duration(float d)
    {
        duration = d;
        return this;
    }

    public UITweener Delay(float d)
    {
        delay = d;
        return this;
    }

    public UITweener Curve(AnimationCurve curve)
    {
        animationCurve = curve;
        return this;
    }

    public UITweener FinishEvent(System.Action action)
    {
        onFinished = action;
        return this;
    }
